...
int n;
ii a[maxn];
int p[maxn][maxn*maxn], f[maxn][maxn*maxn];
...
void _main() {
    ...
    fto (k, 0, n) fto (c, 0, cap) p[k][c] = f[k][c] = -oo;
    p[0][0] = f[0][0] = 0;
    ...
    fto (i, 1, n) {
        fto (k, 1, n) {
            fto (c, 1, cap) {
                f[k][c] = p[k][c];
                if (a[i].ff <= c) f[k][c] = max(f[k][c], p[k-1][c - a[i].ff] + a[i].ss);
            }
        }
        ...
        if (i == n) {
            fto (k, 1, n) {
                double ans = 0;
                fto (c, 1, cap) ans = max(ans, min(1.0*c, (sum - f[k][c])/2.0 + f[k][c]));
                cout << fixed << setprecision(9) << ans << ' ';
            }
            return;
        }

        fto (k, 1, n) fto (c, 1, cap) p[k][c] = f[k][c];
    }
}
