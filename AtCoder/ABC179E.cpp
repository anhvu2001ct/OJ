...
void _main() {
    ...
    vector<ll> sum(sz(num)+1);
    fto (i, 1, sz(num)) sum[i] = sum[i-1] + num[i-1];

    if (n <= sz(num)) bug(sum[n]);
    else {
        int remain = sz(num) - last + 1;
        n = n-cnt+1;
        ll ans = (sum.back() - sum[last-1]) * (n/remain);
        n %= remain;
        if (n) ans += sum[last+n-1] - sum[last-1];
        bug(ans + sum.back());
    }
}
