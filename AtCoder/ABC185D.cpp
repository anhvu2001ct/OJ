...
void _main() {
    ...
    int width = n;
    if (a[1] - 1 != 0) width = min(width, a[1] - 1);
    if (n - a[m] != 0) width = min(width, n - a[m]);

    fto (i, 2, m) {
        if (a[i] - a[i-1] - 1 != 0) width = min(width, a[i] - a[i-1] - 1);
    }

    int ans = (a[1] - 1 + width - 1)/width + (n - a[m] + width - 1)/width;
    fto (i, 2, m) {
        ans += (a[i] - a[i-1] - 1 + width - 1)/width;
    }
